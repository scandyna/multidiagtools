...
void ArticleConnectionTest::initTestCase()
{
  /*
...
void ArticleConnectionTest::cleanupTestCase()
{
  pvDatabase.close();
}
...
void ArticleConnectionTest::pkTest()
{
   ...
   * Get from QVariant
   */
  pk = ArticleConnectionPk::fromQVariant(10);
  QVERIFY(!pk.isNull());
  QCOMPARE(pk.id(), 10);
  pk = ArticleConnectionPk::fromQVariant(QVariant());
  QVERIFY(pk.isNull());
}
...
void ArticleConnectionTest::createDatabaseSchema()
{
