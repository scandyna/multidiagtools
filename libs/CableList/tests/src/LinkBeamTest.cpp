...
void LinkBeamTest::initTestCase()
{
  /*
...
void LinkBeamTest::cleanupTestCase()
{
  pvDatabase.close();
}
...
void LinkBeamTest::pkTest()
{
   ...
   */
  QVERIFY(pk.isNull());
  pk = LinkBeamPk::fromQVariant(2);
  QVERIFY(!pk.isNull());
  QCOMPARE(pk.id(), 2);
  pk = LinkBeamPk::fromQVariant(QVariant());
  QVERIFY(pk.isNull());
}
...
void LinkBeamTest::createDatabaseSchema()
{
