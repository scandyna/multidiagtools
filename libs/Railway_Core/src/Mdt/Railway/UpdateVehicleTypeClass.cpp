...
//     setLastError(request, mRepository->lastError());
    buildAndNotifyError(request.transactionId, data, mRepository->lastError());
    return false;
  }
...
bool UpdateVehicleTypeClass::checkRequest(const UpdateVehicleTypeClassRequest& request)
{
  ...
  if(!validator.validateId(request.id)){
...
void UpdateVehicleTypeClass::buildAndNotifyError(int transactionId, const VehicleTypeClassData & data, const Mdt::Error & error)
{
...
// void UpdateVehicleTypeClass::setLastError(const UpdateVehicleTypeClassRequest & request, const Error& error)
// {
//   mLastError = error;
