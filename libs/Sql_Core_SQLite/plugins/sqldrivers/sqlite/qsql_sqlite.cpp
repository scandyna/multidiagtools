...
bool QSQLiteDriver::open(const QString & db, const QString &, const QString &, const QString &, int, const QString &conOpts)
{
    ...
    /// new
    bool useExtendedResultCodes = false;
    ...
    for (auto option : opts) {
        ...
        if (option.startsWith(QLatin1String("QSQLITE_BUSY_TIMEOUT"))) {
            ...
            sharedCache = true;
        } else if (option == QLatin1String("QSQLITE_USE_EXTENDED_RESULT_CODES")) {
            useExtendedResultCodes = true;
        }
    }
    ...
    if (sqlite3_open_v2(db.toUtf8().constData(), &d->access, openMode, NULL) == SQLITE_OK) {
        ...
        if (useExtendedResultCodes) {
          if (sqlite3_extended_result_codes(d->access, 1) != SQLITE_OK) {
            setLastError(qMakeError(d->access, tr("Error enabling extended result codes"),
