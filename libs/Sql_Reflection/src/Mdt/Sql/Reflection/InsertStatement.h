...
namespace Mdt{ namespace Sql{ namespace Reflection{
  ...
  namespace Impl{
    ...
    bool mustAddValueToStatement(const Value & value)
    {
    ...
    /*! \internal
     */
    template<typename Statement, typename Pk>
    struct AddValueToStatement
    {
      AddValueToStatement(Statement & statement)
      ...
      void operator()(const FieldValuePair & p) const
      {
        ...
        if( mustAddValueToStatement<Pk, typename FieldValuePair::first_type>(p.second) ){
  ...
  InsertStatement insertStatementFromReflected(const Struct & data)
  {
    ...
    Impl::AddValueToStatement<InsertStatement, PrimaryKey> f(statement);
