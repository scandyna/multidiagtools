...
void cancelRemoveRows(AbstractEditableCachedTableModel *model, const RowRange & rowRange)
{
  Q_ASSERT(model != nullptr);
  Q_ASSERT(rowRange.isValid());
  Q_ASSERT(rowRange.lastRow() < model->rowCount());
...
void cancelRemoveRows(AbstractEditableCachedTableModel *model, const RowSelection & selection)
{
  Q_ASSERT(model != nullptr);
  Q_ASSERT(!selection.isEmpty());
...
void cancelRemoveRows(AbstractEditableCachedTableModel *model, const QItemSelectionModel * selectionModel)
{
  Q_ASSERT(model != nullptr);
  Q_ASSERT(selectionModel != nullptr);
