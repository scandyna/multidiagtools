...
bool qCompare(const Mdt::Container::ColumnList& actualList, const Mdt::Container::ColumnList& expectedList,
              const char* actual, const char* expected, const char* file, int line)
{
  const int actualSize = actualList.size();
  const int expectedSize = expectedList.size();
  if(actualSize != expectedSize){
    const QString msg = QString::fromLatin1("Compared column lists have different column counts.\n"
                                            "   Actual   (%1) columns: %2\n"
  ...
  for(int i = 0; i < n; ++i){
    if(actualList.at(i) != expectedList.at(i)){
      const auto msg = QString::fromLatin1("Compared column lists differs at index %1.\n"
                                           "   Actual   (%2): %3\n"
                                           "   Expected (%4): %5")
                                           .arg(i)
