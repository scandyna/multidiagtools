...
At first, the lock file name will be buit for each lock directories found. Then, is no lock file is found, the port will be opened with open(). Just after open, the port will be locked with fcntl(). Note: to close opend port, the system call close() can be used. \begin{DoxyReturn}{Returns}
