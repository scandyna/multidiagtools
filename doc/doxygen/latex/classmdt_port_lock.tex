...
\begin{Desc}
\item[\hyperlink{todo__todo000004}{Todo}]Create serious test cases for this class \end{Desc}
...
\index{mdtPortLock@{mdtPortLock}!isLockedByAnother@{isLockedByAnother}}
\index{isLockedByAnother@{isLockedByAnother}!mdtPortLock@{mdtPortLock}}
\subsubsection[{isLockedByAnother}]{\setlength{\rightskip}{0pt plus 5cm}bool mdtPortLock::isLockedByAnother (
...
Returns true if port is locked by another port instance. 

The rule is simple:
\begin{DoxyItemize}
\item If port is open and locked, false will be returned.
\item If port is closed and locked, true will be returned. 
\end{DoxyItemize}\hypertarget{classmdt_port_lock_a25e9a2cb8a10069fae75c76b6809c27f}{
...
At first, the lock file name will be buit for each lock directories found. Then, is no lock file is found, the port will be opened with open(). Just after open, the port will be locked with fcntl(). Note: to close opend port, the system call close() can be used.
...
NOTE:\begin{Desc}
\item[\hyperlink{todo__todo000001}{Todo}]Flag pvIsLocked : could be locked by another (other process, or other thread in this process), true seems not to be correct here ! \end{Desc}


NOTE:\begin{Desc}
\item[\hyperlink{todo__todo000002}{Todo}]Flag pvIsLocked ? \end{Desc}


NOTE:\begin{Desc}
\item[\hyperlink{todo__todo000003}{Todo}]Flag pvIsLocked ? \end{Desc}
...
Will remove lockfiles and unlock port with fcntl(). Note: use close() if file is not used later, this preserve a system call. 
