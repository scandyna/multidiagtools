
#include "mdtSqlFieldHandler.h"

mdtSqlFieldHandler::mdtSqlFieldHandler(const QObject & parent) {
}

mdtSqlFieldHandler::~mdtSqlFieldHandler() {
}

void mdtSqlFieldHandler::setField(const QSqlField & field) {
}

void mdtSqlFieldHandler::setDataWidget(const QLineEdit & widget) {
}

void mdtSqlFieldHandler::setDataWidget(const QAbstractButton & widget) {
}

void mdtSqlFieldHandler::setDataWidget(const QDateTimeEdit & widget) {
}

void mdtSqlFieldHandler::setDataWidget(const QDoubleSpinBox & widget) {
}

void mdtSqlFieldHandler::setDataWidget(const QSpinBox & widget) {
}

void mdtSqlFieldHandler::clear() {
}

bool mdtSqlFieldHandler::isNull() {
}

bool mdtSqlFieldHandler::dataWasEdited() {
}

bool mdtSqlFieldHandler::checkBeforeSubmit() {
}

void mdtSqlFieldHandler::dataEdited() {
}

void mdtSqlFieldHandler::onCurrentIndexChanged(int index) {
}

void mdtSqlFieldHandler::onDataEdited(const QString & text) {
}

void mdtSqlFieldHandler::onDataEdited(bool state) {
}

void mdtSqlFieldHandler::onDataEdited(const QDateTime & datetime) {
}

