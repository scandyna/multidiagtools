...
static int  mxuport_startup(struct usb_serial *serial)
{
	 ...
	 */
	spin_lock_init(&mx_serial->bus_splock);
...
static int  mxuport_open(struct usb_serial_port *port, struct file *filp)
{
	 ...
	 */
	//mx_port->write_urb = usb_alloc_urb(0, GFP_KERNEL);
...
static int mxuport_ioctl (struct usb_serial_port *port, struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    printk(KERN_DEBUG "[File %s , line %d] entering mxuport_ioctl(): cmd: %u , arg: %lu\n", __FILE__ , __LINE__ , cmd, arg);
	...
	printk(KERN_DEBUG "[File %s , line %d] switch cmd: %u\n", __FILE__ , __LINE__ , cmd);
	...
	switch(cmd){
		...
		case TIOCSSOFTCAR:			
			...
			if(get_user(mask, (unsigned int *)arg))
			    return -EFAULT;
		...
		default:
			printk(KERN_DEBUG "[File %s , line %d] unknow cmd: %u\n", __FILE__ , __LINE__ , cmd);
						      ...
						      struct serial_struct __user *ret_arg)
{
	...
	printk(KERN_DEBUG "[File %s , line %d] entering mx_get_serial_info()\n", __FILE__ , __LINE__);
