 ...
 *
 * (06/10/2005) Danny Lin (danny.lin@moxa.com.tw)
 * 	merged the header file from kernel 2.6.12 to 2.6.17
 *
...
#define SERIAL_TTY_MAJOR	    232	    
#define SERIAL_TTY_MINORS	    255	    /* loads of devices :) */

#define MAX_NUM_PORTS		    16	    /* The maximum number of ports one device can grab at once */
...
#else
	int  (*ioctl)		(struct usb_serial_port *port, unsigned int cmd, unsigned long arg);
#endif
...
#else
	int (*break_ctl)	(struct usb_serial_port *port, int break_state);
#endif
...
#else
	int  (*tiocmget)	(struct usb_serial_port *port);
	int  (*tiocmset)	(struct usb_serial_port *port, unsigned int set, unsigned int clear);
#endif
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,19))
...
extern int  mx_usbserial_register(struct usb_serial_driver *driver);
extern void mx_usbserial_deregister(struct usb_serial_driver *device);
#else
#define to_usb_serial_driver(d) container_of(d, struct usb_serial_device_type, driver)
extern int  mx_usbserial_register(struct usb_serial_device_type *driver);
extern void mx_usbserial_deregister(struct usb_serial_device_type *device);
#endif

#if (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,19))
extern void mx_usbserial_port_softint(struct work_struct *work);
#else
extern void mx_usbserial_port_softint(void *private);
#endif

//extern int usb_serial_init(void);
//extern void usb_serial_exit(void);
extern int mx_usbserial_probe(struct usb_interface *iface, const struct usb_device_id *id);
extern void mx_usbserial_disconnect(struct usb_interface *iface);
...
extern void mx_usbserial_console_init (int debug, int minor);
extern void mx_usbserial_console_exit (void);
#else
static inline void mx_usbserial_console_init (int debug, int minor) { }
static inline void mx_usbserial_console_exit (void) { }
#endif

/* Functions needed by other parts of the usbserial core */
extern struct usb_serial *mx_usbserial_get_by_index (unsigned int minor);

#if (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,14))
extern int mx_usbserial_bus_register (struct usb_serial_driver *driver);
extern void mx_usbserial_bus_deregister (struct usb_serial_driver *driver);
#else
extern int mx_usbserial_bus_register (struct usb_serial_device_type *driver);
extern void mx_usbserial_bus_deregister (struct usb_serial_device_type *driver);
#endif

extern struct bus_type mx_usb_serial_bus_type; 
extern struct tty_driver *mx_usbserial_tty_driver;
